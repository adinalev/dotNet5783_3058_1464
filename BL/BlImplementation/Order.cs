...
internal class Order : BlApi.IOrder
{
    ...
    public void UpdateOrder(BO.Order order) //Update Method, this time with logic checks
    {
    ...
    public BO.Order DeliveredUpdate(int orderID)
    {
        ...
        if (order.ID == orderID && order.DeliveryDate < DateTime.Today)//if oId exists and has not been shipped 
        {
            DO.Order ord = new()
            {
                ID = orderID,
    ...
    {
        DO.Order order = dal.dalOrder.GetByID(orderID);//get the order from DO of orderId-or catch exception
        if (order.ID == orderID && order.ShippingDate < DateTime.Today)//if oId exists and has not been shipped 
        {
            DO.Order ord = new()
            {
                ID = orderID,
